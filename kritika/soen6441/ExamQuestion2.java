...
public class ExamQuestion2 {
	...
	public static void main(String[] args) {
		...
		Map<Boolean, List<Course>> partitionedCourses = courses.stream()
									.collect(partitioningBy(c -> c.getProgram().equals(Program.SOEN)));
		System.out.println("b) " + partitionedCourses);
		
		// Subtask c)
		/*final Course courseLongestName = courses.stream()
							.reduce((s1,s2)->(s1.getName().length()>s2.getName().length())?s1:s2).get();*/
